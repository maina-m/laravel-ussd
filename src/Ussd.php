...
class Ussd
{
    ...
    private function operate(): array
    {
        ...
        $record = new Record($this->storeName, $this->context->session_id(), $this->context->msisdn());
        ...
        if ($rid = $record->get(static::HEAL)) {
            $record = new Record($this->storeName, $rid, $this->context->msisdn());
        }
        ...
        if ($record->has(static::INIT)) {
            ...
            if (ContinuingMode::START !== $this->continuingMode) {
                $record->set(static::SPUR, $this->context->session_id(), $this->continuingTtl, true);
            }
        }
